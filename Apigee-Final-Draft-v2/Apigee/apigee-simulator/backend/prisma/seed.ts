...
async function main() {
  ...
  // Demonstrates BasicAuthentication's "Validate Against KVM" simulator extension: credentials
  // sent as a real HTTP Basic Authorization header are decoded and checked against a KVM entry,
  // not just extracted into flow variables. Org-scoped so it resolves the same in every environment.
  let basicAuthKvm = await prisma.kvm.findFirst({ where: { name: "basic-auth-creds", scope: "organization", environmentId: null } });
  if (!basicAuthKvm) {
    basicAuthKvm = await prisma.kvm.create({ data: { name: "basic-auth-creds", scope: "organization", encrypted: true } });
  }
  await prisma.kvmEntry.upsert({ where: { kvmId_key: { kvmId: basicAuthKvm.id, key: "username" } }, update: {}, create: { kvmId: basicAuthKvm.id, key: "username", value: "apiuser" } });
  await prisma.kvmEntry.upsert({ where: { kvmId_key: { kvmId: basicAuthKvm.id, key: "password" } }, update: {}, create: { kvmId: basicAuthKvm.id, key: "password", value: "S3cretPass!23" } });

  await createProxy({
    name: "KvmBasicAuthDemo-v1",
    basePath: "/kvm-secure",
    description: 'Guarded by BasicAuthentication with "Validate Against KVM" enabled - send Authorization: Basic base64(apiuser:S3cretPass!23) to pass (credentials live in the "basic-auth-creds" KVM, see the KVM module).',
    targetServerName: "CustomerService",
    policies: [{ name: "BA-ValidateKvm", type: "BasicAuthentication", phase: "preRequest", overrides: { operation: "Decode", validateAgainstKvm: true, kvmName: "basic-auth-creds", kvmUsernameKey: "username", kvmPasswordKey: "password" } }],
    deployEnvs: ["dev", "test", "prod"],
  });
